...
export const Register = async (req, res) => {
    ...
    if (!username || !password || !email || !confirmPassword) return res.status(400).json({msg: "Silahkan isi semua field"});
...
export const Login = async (req, res) => {
    try {
        ...
        const accesstoken = jwt.sign({ userId, userName, userEmail }, process.env.ACCESS_TOKEN, { expiresIn: '30s' });
        const refreshtoken = jwt.sign({ userId, userName, userEmail }, process.env.REFRESH_TOKEN, { expiresIn: '1d' });
